        ...
        <nav id="sidebar" class="text-white p-4" style="background-color: var(--color_menu);">
            ...
            <ul class="nav flex-column">
                ...
                @can('v_asistente_general')
                    <li class="nav-item {{ request()->segment(1) == 'pacientes' ? 'active' : '' }}">
                        <a class="nav-link {{ request()->segment(1) == 'pacientes' ? 'active' : '' }}"
                            href="{{ route('usuarios.pacientes', 'paciente') }}">Lista de pacientes</a>
                    </li>
                @endcan
                @can('v_asistente_general')
                    <li class="nav-item {{ request()->segment(1) == 'usuarios' ? 'active' : '' }}">
                        <a class="nav-link {{ request()->segment(1) == 'usuarios' ? 'active' : '' }}"
                            href="{{ route('usuarios.index', 'usuario') }}">Lista de usuarios</a>
                    </li>
                @endcan
                @can('v_admnistrador')
                    <li class="nav-item {{ request()->segment(1) == 'agenda' ? 'active' : '' }}">
                        <a class="nav-link {{ request()->segment(1) == 'agenda' ? 'active' : '' }}"
                            href="{{ route('agenda.index') }}">Mi agenda</a>
                    </li>
                @endcan
